...
class GlassGUI extends AbstractGUI{
  ...
  private val translation = new Translation3DGUI(XRATIO, YRATIO, ZRATIO)
      ...
      data match {
        case (x: Float, y: Float, z: Float) =>
          if (last_qua != null) {
            ...
            val nx=(y+0.15f)*2
            val nz= -z*2
            panel.updateLines(translation.translateToBox((nx, ny, nz), BOX_SIZE, last_qua).map {
              case (x, y) =>
...
object GlassGUI extends SimpleSwingApplication {
  private val gui = new GlassGUI
